...
func GenerateCollectionGrid(c *gin.Context) {
	...
	// Downloading + FFmpeg frame extraction for 12 cards sequentially is slow.
	// All cards are independent, so we fetch them concurrently then render.
	client := &http.Client{Timeout: 12 * time.Second}

	type fetchedCard struct {
		img image.Image // nil if download failed
		err error
	}
	fetchResults := make([]fetchedCard, len(req.Images))
	...
	// Render all cards onto the canvas (single-threaded, gg is not goroutine-safe)
	for i, card := range req.Images {
