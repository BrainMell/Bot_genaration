...
=======
        // Scale to 720x720 square for WhatsApp
        if len(animInputs) > 0 {
                lastIdx := len(filterParts) - 1
                filterParts[lastIdx] = strings.Replace(filterParts[lastIdx], "[final]", "[prescale]", 1)
                filterParts = append(filterParts, "[prescale]scale=720:720:force_original_aspect_ratio=increase,crop=720:720[final]")
>>>>>>> 36c8a64 (fix: re-apply lost Go fixes + player/summon formation + facing)
        }
