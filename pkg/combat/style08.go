...
func s08Skillup(req *portraitRequest) *gg.Context {
	...
	y := s08Marquee(dc, W, 34, W-80, 66, skillTitle, strings.ToUpper(cardstyle.Sanitize(styledName(req))), p)
...
// s08Evolve - class ascension card. Same design language as this style's
...
func s08Evolve(req *portraitRequest) *gg.Context {
	e := *req
	e.DocTitle = "CLASS UPGRADED"
	if e.SubLabel == "" {
		e.SubLabel = "EVOLVED"
	}
	return s08Skillup(&e)
