...
func s08Equip(req *portraitRequest) *gg.Context {
	...
	// hero HUD panel
	s08HUDPanel(dc, 50, y, 400, 720, p, "OPERATIVE")
	drawHeroFitted(dc, req, 250, y+300, 320, 340, *p)
	cardstyle.DotLeader(dc, 100, 400, y+520, 1, cardstyle.HexA(0x40e0ff, 90))
	cardstyle.Text(dc, cardstyle.FtPS2P, 12, strings.ToUpper(cardstyle.Sanitize(styledName(req))), 250, y+556, cardstyle.Hex(0xe0f2ff), 0.5, 0.5, 340, 8)
	if req.SealText != "" {
		cardstyle.Text(dc, cardstyle.FtPS2P, 9, strings.ToUpper(cardstyle.Sanitize(req.SealText))+"-RANK", 250, y+590, cardstyle.HexA(0x40e0ff, 235), 0.5, 0.5, 200, 7)
	}
	cardstyle.Text(dc, cardstyle.FtInter, 11, cardstyle.TruncateRunes(cardstyle.Sanitize(req.Caption), 52), 250, y+630, cardstyle.HexA(0x80a8d0, 230), 0.5, 0.5, 340, 8)

	// 9 slot panels
	gx, gy, gw, gh, gap := 500.0, y+16, 300.0, 216.0, 20.0
	for i, s := range req.Slots {
		...
		dc.SetColor(fill)
		dc.DrawRoundedRectangle(x, yy, gw, gh, 4)
		dc.Fill()
		cardstyle.BracketCorners(dc, x, yy, gw, gh, 12, cardstyle.HexA(0x40e0ff, cardstyleIfByte(empty, 80, 180)), 1.4)
		slotCode := strings.ToUpper(cardstyle.Sanitize(s.Slot))
		if len([]rune(slotCode)) > 4 {
			slotCode = string([]rune(slotCode)[:4])
		}
		cardstyle.Text(dc, cardstyle.FtPS2P, 9, slotCode, x+14, yy+24, cardstyle.HexA(0x80a8d0, 235), 0, 0.5, 100, 7)
		for t := 0; t < s.Tier && t < 5; t++ {
			dc.SetColor(cardstyle.HexA(0xff40a0, 235))
			dc.DrawRectangle(x+gw-26-float64(t)*16, yy+16, 8, 8)
			dc.Fill()
		}
		name := "EMPTY"
		...
		cardstyle.Text(dc, cardstyle.FtPS2P, 10, cardstyle.TruncateRunes(strings.ToUpper(name), 16), x+14, yy+64, ncol, 0, 0.5, gw-28, 7)
		if s.TierLabel != "" {
			cardstyle.Text(dc, cardstyle.FtInter, 10, strings.ToUpper(cardstyle.Sanitize(s.TierLabel)), x+14, yy+90, cardstyle.HexA(0xff9ecb, 220), 0, 0.5, gw-28, 8)
		}
		if !empty && s.DurMax > 0 {
			frac := s.Dur / s.DurMax
			if frac > 1 {
			...
			cardstyle.SegBar(dc, x+14, yy+gh-40, gw-28, 9, frac, 10, cardstyle.HexA(0x40e0ff, 220), cardstyle.HexA(0x40e0ff, 36))
			cardstyle.Text(dc, cardstyle.FtInter, 10, cardstyle.FmtF(s.Dur)+" / "+cardstyle.FmtF(s.DurMax), x+14, yy+gh-18, cardstyle.HexA(0x80a8d0, 220), 0, 0.5, gw-28, 8)
		}
	}
	return dc
}
