...
func GenerateCombatImage(c *gin.Context) {
        ...
        // (spark_15.png) is THE default PvP background. Previously PvP without an
...
func drawOverheadHPBar(dc *gg.Context, x, y, w, h float64, pct float64, fillImg image.Image) {
        ...
        dc.SetRGBA(0.04, 0.04, 0.06, 0.88)
        dc.DrawRoundedRectangle(x, y, w, h, 3)
        dc.Fill()
        // Fill (hp5 texture) — only when there is HP left
        if pct > 0 {
                innerW := w - 2
                innerH := h - 2
                fillW := int(innerW * pct)
                if fillW < 1 {
                        fillW = 1
                }
                fill := imaging.Resize(fillImg, fillW, int(innerH), imaging.NearestNeighbor)
                dc.DrawImage(fill, int(x+1), int(y+1))
        }
        // Border
