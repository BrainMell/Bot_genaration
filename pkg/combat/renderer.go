...
func GenerateCombatImage(c *gin.Context) {
        ...
        // 💀 CAPACITY SWAP (2026-09-20): owner ruling — the background must react
        // to the number and mass of the combatants. Small stages (env1/2/3 cave
        // platforms) cannot host a large party: probe the counted formation with
        // unit widths and swap to the stage's declared overflow arena when a side
        // cannot stand everyone. PvE only (PvP arenas are fixed wide stages).
        if req.CombatType != "PVP" {
                visP, visE, visS, hasBoss := countVisibleCombatants(req)
                if suggest := groundOverflowIfNeeded(filepath.Base(bgPath), visP, visE, visS, hasBoss); suggest != "" {
                        cand := filepath.Join(assetsPath, "rpgasset", "environment", suggest)
                        if fileExists(cand) {
                                log.Printf("[groundmap] capacity: %s cannot hold %dv%d (+%d summons, boss=%v) -> overflow to %s",
                                        filepath.Base(bgPath), visP, visE, visS, hasBoss, suggest)
                                bgPath = cand
                        }
                }
        }
        ...
        // 💀 FORMATION (2026-09-20): resolve ALL enemy sprites BEFORE assigning
        // positions, so the ground-map formation can pack them by REAL post-resize
        // width (mass-aware arrangement: wide bosses consume more frontage, rows
        // wrap onto the next measured ground line when a row is full).
        type enemySpriteItem struct {
                img       image.Image
                w, h      int
                hpPerc    float64
                origIndex int
        ...
        for i, enemy := range req.Enemies {
                ...
                w, h := eSprite.Bounds().Dx(), eSprite.Bounds().Dy()
                eItems = append(eItems, enemySpriteItem{img: eSprite, w: w, h: h, hpPerc: hpPerc, origIndex: i, name: enemy.Name})
                eWidths = append(eWidths, float64(w))
        }

        // 💀 GROUND MAP formation plan: width-aware packing into THIS background's
        // standable enemy zone. Falls back to the legacy slot table (still
        // ground-adjusted) when the background has no formation data.
        enemySlots := groundPlanFormation(bgName, "enemy", eWidths)
        for idx, it := range eItems {
                var feetX, feetY float64
                if enemySlots != nil && idx < len(enemySlots) {
                        feetX, feetY = enemySlots[idx].X, enemySlots[idx].Y
                } else {
                        // Position from EnemySlots table - no inline math.
                        slot := slotFor(EnemySlots, idx)
                        feetX = slot.X
                        ...
                        feetY = groundAdjustedY(bgName, "enemy", slot.Y)
                }

                // Convert feet position to draw position (top-left anchor for DrawImage)
                ex := feetX - float64(it.w)/2
                ey := feetY - float64(it.h)

                mobQueue = append(mobQueue, RenderItem{img: it.img, x: ex, y: ey, feetY: feetY, hpPercent: it.hpPerc, origIndex: it.origIndex, name: it.name})
        }
        ...
        // 💀 FORMATION (2026-09-20): collect → plan by real width → draw, so
        // summons pack the background's measured summon zone (cave platforms:
        // no more summons hovering over lava at legacy X=60).
        // Slot table (SummonSlots) remains the legacy fallback + crop-then-resize-by-height (75px).
        ...
        if req.CombatType != "PVP" {
                ...
                for i, summon := range req.Summons {
                        ...
                        w, h := sSprite.Bounds().Dx(), sSprite.Bounds().Dy()
                        sItems = append(sItems, summonSpriteItem{img: sSprite, w: w, h: h, origIndex: i, name: summonName, maxHP: summon.MaxHP, curHP: summon.CurrentHP})
                        sWidths = append(sWidths, float64(w))
                }

                // 💀 GROUND MAP formation plan for the summon zone.
                summonSlots := groundPlanFormation(bgName, "summon", sWidths)
                for idx, it := range sItems {
                        var summonFeetX, summonFeetY float64
                        if summonSlots != nil && idx < len(summonSlots) {
                                summonFeetX, summonFeetY = summonSlots[idx].X, summonSlots[idx].Y
                        } else {
                                // Position from SummonSlots table - no inline math.
                                slot := slotFor(SummonSlots, idx)
                                summonFeetX = slot.X
                                // 💡 GROUND MAP (2026-09-20): per-background summon depth rows.
                                summonFeetY = groundAdjustedY(bgName, "summon", slot.Y)
                        }

                        // Convert feet to draw position
                        sx := summonFeetX - float64(it.w)/2
                        sy := summonFeetY - float64(it.h)

                        // Shadow at feet
                        utils.DrawShadow(dc, summonFeetX, summonFeetY-2, float64(it.w)*0.6, 0.85)

                        // Turn indicator - floating crystal above head
                        if isAttacker("summon", it.origIndex) {
                                queueTurnCrystal(summonFeetX, sy, float64(it.h))
                        }

                        // Sprite
                        dc.DrawImage(it.img, int(sx), int(sy))

                        // HP bar
                        hpBarTopY := sy - 12
                        if it.maxHP > 0 && it.curHP > 0 {
                                uiPath2 := func(f string) string { return filepath.Join(assetsPath, "rpgasset", "ui", f) }
                                hpBarImg, err := utils.LoadImage(uiPath2("hp5.png"))
                                if err == nil {
                                        barW := 70.0
                                        barH := 10.0
                                        hpPerc := float64(it.curHP) / float64(it.maxHP)
                        ...
                        // Nameplate pill above HP bar - queued for the collision pass
                        pillQueue = append(pillQueue, pillItem{cx: summonFeetX, bottomY: hpBarTopY - 2, name: it.name})
                }
        ...
        if req.CombatType == "PVP" && !(req.CombatType == "PVP" && len(req.Players) >= 2 && req.Players[0].Mode == "summon" && req.Players[0].Species != "" && req.Players[1].Mode == "summon" && req.Players[1].Species != "") && len(req.Summons) > 0 {
                ...
                for i, summon := range req.Summons {
                        ...
                        // 💀 GROUND MAP (2026-09-20): keep the fixed PvP summon X on
                        // this background's standable summon zone (cave-safe).
                        summonFeetX := groundClampX(bgName, "summon", pos.x)
                        summonFeetY := pos.y
        ...
        if isPvPSummonDuel {
                ...
                // 💀 GROUND MAP (2026-09-20): fixed duel X clamped into the
                // background's standable player/enemy zones (cave-safe).
                pvpSummonPositions := []struct{ x, y int }{
                        {int(groundClampX(bgName, "player", 220)), pvpSummonFeetY},
                        {int(groundClampX(bgName, "enemy", 800)), pvpSummonFeetY},
                }
        ...
        if len(req.Players) > 0 {
                ...
                if req.CombatType == "PVP" && !(req.CombatType == "PVP" && len(req.Players) >= 2 && req.Players[0].Mode == "summon" && req.Players[0].Species != "" && req.Players[1].Mode == "summon" && req.Players[1].Species != "") {
                        ...
                        //   At Y=445, shadow bottom=461, 8px clear of panel top. ✅
                        // 💀 GROUND MAP (2026-09-20): per-background PvP feet line +
                        // X clamped into the standable player/enemy zone.
                        ...
                        drawPvPFighter(p, int(groundClampX(bgName, "player", 220)), pvp1v1FeetY, p1Flip, isAttacker("player", 0))
                        if len(req.Players) > 1 {
                                ...
                                drawPvPFighter(req.Players[1], int(groundClampX(bgName, "enemy", 800)), pvp1v1FeetY, p2Flip, isAttacker("player", 1))
                        }
                        ...
                        // 💀 FORMATION (2026-09-20): collect ALL player sprites first so
                        // the ground-map formation can pack them by REAL width into this
                        // background's standable player zone (rows wrap when full).
                        type playerSpriteItem struct {
                                img       image.Image
                                w, h      int
                                origIndex int
                                name      string
                        ...
                        for pi, cp := range req.Players {
                                ...
                                w, h := cpSprite.Bounds().Dx(), cpSprite.Bounds().Dy()
                                pItems = append(pItems, playerSpriteItem{img: cpSprite, w: w, h: h, origIndex: pi, name: cpName, maxHP: cp.MaxHP, curHP: cp.CurrentHP})
                                pWidths = append(pWidths, float64(w))
                        }

                        // 💀 GROUND MAP formation plan (width-aware, per-background).
                        playerSlots := groundPlanFormation(bgName, "player", pWidths)
                        for idx, it := range pItems {
                                var cpFeetX, cpFeetY float64
                                if playerSlots != nil && idx < len(playerSlots) {
                                        cpFeetX, cpFeetY = playerSlots[idx].X, playerSlots[idx].Y
                                } else {
                                        cpSlot := slotFor(PlayerSlots, idx)
                                        cpFeetX = cpSlot.X
                                        // 💡 GROUND MAP (2026-09-20): per-background player depth rows.
                                        cpFeetY = groundAdjustedY(bgName, "player", cpSlot.Y)
                                }
                                cpX := cpFeetX - float64(it.w)/2
                                cpY := cpFeetY - float64(it.h)

                                playerQueue = append(playerQueue, PlayerRenderItem{
                                        img: it.img, x: cpX, y: cpY,
                                        feetX: cpFeetX, feetY: cpFeetY,
                                        spriteW:   float64(it.w),
                                        origIndex: it.origIndex, name: it.name,
                                        maxHP: it.maxHP, curHP: it.curHP,
                                })
                        }
