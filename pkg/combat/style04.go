...
// s04BrandMark - branding-iron workshop mark burned into the wood.
func s04BrandMark(dc *gg.Context, W float64, y0, y1 float64, p *cardstyle.Palette, label string) {
	if y1-y0 < 90 {
		return
	}
	cy := (y0 + y1) / 2
	// burned ring + crossed tools
	burn := cardstyle.N(46, 26, 12, 220)
	hi := cardstyle.N(232, 190, 130, 60)
	cardstyle.Ring(dc, W/2, cy, 34, burn, 4)
	cardstyle.Ring(dc, W/2, cy-1.5, 34, hi, 1)
	dc.SetLineWidth(4)
	dc.SetColor(burn)
	dc.DrawLine(W/2-14, cy+14, W/2+14, cy-14)
	dc.Stroke()
	dc.DrawLine(W/2-14, cy-14, W/2-4, cy-4)
	dc.Stroke()
	// hammer head
	dc.DrawRectangle(W/2+8, cy-20, 12, 9)
	dc.Fill()
	if label != "" {
		cardstyle.Text(dc, cardstyle.FtMedieval, 14, strings.ToUpper(cardstyle.Sanitize(label)), W/2, cy+58, cardstyle.N(116, 82, 50, 230), 0.5, 0.5, W-160, 10)
	}
}
...
func s04Skillup(req *portraitRequest) *gg.Context {
	...
	y := s04Signboard(dc, W, 30, 360, 62, skillTitle, styledName(req), p)
	...
	s04BrandMark(dc, W, py+160, H-100, p, "CRAFTED WITH CARE")
	s04Footer(dc, W, H, req, p)
	return dc
}
...
func s04Abilities(req *portraitRequest) *gg.Context {
	...
	s04BrandMark(dc, W, iy+16, H-96, p, "")
	s04Footer(dc, W, H, req, p)
	return dc
}
...
func s04Equip(req *portraitRequest) *gg.Context {
	...
	// 9 pegged tags hanging from two wooden rails (alternating drop lengths)
	gx, gy, gw, gh := 500.0, 196.0, 300.0, 196.0
	for i, s := range req.Slots {
		col, row := i%3, i/3
		x := gx + float64(col)*(gw+22)
		railY := gy + float64(row)*(gh+34) - 26
		// rail drawn once per row (leftmost tag of the row draws it)
		if col == 0 {
			dc.SetColor(cardstyle.N(64, 40, 20, 255))
			dc.DrawRoundedRectangle(gx-16, railY, (gw+22)*2+gw+32, 12, 5)
			dc.Fill()
			dc.SetColor(cardstyle.N(112, 74, 40, 255))
			dc.DrawRoundedRectangle(gx-16, railY, (gw+22)*2+gw+32, 4, 2)
			dc.Fill()
		}
		drop := 12.0
		if col%2 == 1 {
		...
		y := railY + drop
		empty := s.Empty
		// peg + rope from rail to tag
		cardstyle.Rivet(dc, x+gw/2, railY+6, 5, cardstyle.N(46, 42, 38, 255))
		dc.SetColor(cardstyle.N(150, 128, 100, 220))
		dc.SetLineWidth(2)
		dc.DrawLine(x+gw/2-7, railY+10, x+gw/2, y)
		dc.Stroke()
		dc.DrawLine(x+gw/2+7, railY+10, x+gw/2, y)
		dc.Stroke()
...
func s04Shop(req *portraitRequest) *gg.Context {
	...
	if y+ph-24-(iy+16) > 90 {
		s04BrandMark(dc, W, iy+16, y+ph-24, p, "THE WORKSHOP")
	}
	s04Footer(dc, W, H, req, p)
	return dc
}
...
func s04GuildInfo(req *portraitRequest) *gg.Context {
	...
	s04BrandMark(dc, W, ry+120, H-100, p, "")
	s04Footer(dc, W, H, req, p)
	return dc
}

// s04Evolve - class ascension card. Same design language as this style's
...
func s04Evolve(req *portraitRequest) *gg.Context {
	e := *req
	e.DocTitle = "THE NEW SIGN"
	if e.SubLabel == "" {
		e.SubLabel = "EVOLVED"
	}
	return s04Skillup(&e)
