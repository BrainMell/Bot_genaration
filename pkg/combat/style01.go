...
func s01Footer(dc *gg.Context, W, H float64, req *portraitRequest, p *cardstyle.Palette) {
	...
	if req.Caption != "" {
		cardstyle.Text(dc, cardstyle.FtMedieval, 15, cardstyle.TruncateRunes(cardstyle.Sanitize(req.Caption), 56), 96, H-44, cardstyle.WithA(p.SealTx, 215), 0, 0.5, W-150, 10)
	}
}
...
func s01Skillup(req *portraitRequest) *gg.Context {
	...
	s01IronPlate(dc, W, skillTitle, styledName(req), p)
...
func s01GuildInfo(req *portraitRequest) *gg.Context {
	...
	if req.EmblemImg != nil {
		dc.Push()
		dc.DrawCircle(W/2, 240, 62)
		dc.Clip()
		cardstyle.FitEmblem(dc, req.EmblemImg, W/2, 240, 116, 116)
		dc.Pop()
	...
	if req.Caption != "" {
		cardstyle.Text(dc, cardstyle.FtMedieval, 13, cardstyle.TruncateRunes(cardstyle.Sanitize(req.Caption), 52), W/2, H-44, cardstyle.WithA(p.SealTx, 200), 0.5, 0.5, 400, 9)
	}
	return dc
}

// s01Evolve - class ascension card. Same design language as this style's
...
func s01Evolve(req *portraitRequest) *gg.Context {
	e := *req
	e.DocTitle = "REFORGED"
	if e.SubLabel == "" {
		e.SubLabel = "EVOLVED"
