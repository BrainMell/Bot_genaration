...
func s01IronPlate(dc *gg.Context, W float64, title, sub string, p *cardstyle.Palette) {
	...
	cardstyle.BevelRect(dc, W-150, y, 110, h, cardstyle.Darken(p.Accent, 8), cardstyle.Lighten(p.Accent, 30), cardstyle.N(10, 10, 12, 255), 2.2)
	cardstyle.Rivet(dc, W-141, y+9, 3.6, p.Track)
	cardstyle.Rivet(dc, W-51, y+9, 3.6, p.Track)
}
...
func s01Shop(req *portraitRequest) *gg.Context {
	...
	for _, e := range entries {
		...
		cardstyle.Engrave(dc, cardstyle.FtInterSemi, 17, cardstyle.Sanitize(e.Title), 66, y+32, p.Ink, cardstyle.Lighten(p.Panel, 55), 0, 0.5, 420, 11)
		if e.Sub != "" {
			cardstyle.Text(dc, cardstyle.FtInter, 11, cardstyle.TruncateRunes(cardstyle.Sanitize(e.Sub), 70), 66, y+58, cardstyle.WithA(p.Ink, 190), 0, 0.5, 430, 8)
		}
		if e.Runes != "" {
			cardstyle.Spaced(dc, cardstyle.FtCinzel, 13, cardstyle.Sanitize(e.Runes), 66, y+80, cardstyle.N(255, 179, 64, 255), 0, 2.2)
		}
		cardstyle.Tag(dc, W-140, y+24, 84, 34, p.Track, cardstyle.Lighten(p.Track, 46), p.SealTx, cardstyle.Sanitize(e.Value), cardstyle.FtCinzel, 13)
		y += eh + 16
	}
	s01Footer(dc, W, H, req, p)
	return dc
...
func s01GuildInfo(req *portraitRequest) *gg.Context {
	...
	name := styledName(req)
	cardstyle.Engrave(dc, cardstyle.FtCinzelDec, 26, strings.ToUpper(cardstyle.Sanitize(name)), W/2, 402, p.Ink, cardstyle.Lighten(p.Panel, 60), 0.5, 0.5, W-140, 14)
	if req.Motto != "" {
		cardstyle.Text(dc, cardstyle.FtMedieval, 15, "'"+cardstyle.TruncateRunes(cardstyle.Sanitize(req.Motto), 60)+"'", W/2, 434, cardstyle.WithA(p.Ink, 200), 0.5, 0.5, W-120, 10)
	}
