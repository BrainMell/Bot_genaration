...
func renderCombatFrame(req *CombatRequest, fs *frameState, assetsPath string) (image.Image, error) {
	...
	// FIX 2026-09-11 (owner directive): spark_15 arena = default PvP background.
	...
	// FIX 2026-09-11: these are now HEIGHT targets (crop-then-fit-by-height,
	// matching the static renderer exactly) instead of width targets.
	enemySpriteSize := 170.0
	playerSpriteSize := 150.0
	summonSpriteSize := 75.0
	...
	for i, p := range req.Players {
		...
		// FIX 2026-09-11: crop first (matches static renderer), resize by height.
		pSprite = cropToVisibleBounds(pSprite)
		pSprite = imaging.Resize(pSprite, 0, int(playerSpriteSize), imaging.NearestNeighbor)
