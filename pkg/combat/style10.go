...
func s10Skillup(req *portraitRequest) *gg.Context {
	...
	cardstyle.Cartouche(dc, W/2, 74, 440, 58, p.Panel, p.Accent, p.Ink, skillTitle, cardstyle.FtCinzelDec, 22)
...
func s10GuildInfo(req *portraitRequest) *gg.Context {
	...
	if req.EmblemImg != nil {
		dc.Push()
		dc.DrawCircle(W/2, 224, 68)
		dc.Clip()
		cardstyle.FitEmblem(dc, req.EmblemImg, W/2, 224, 124, 124)
		dc.Pop()
...
// s10Evolve - class ascension card. Same design language as this style's
...
func s10Evolve(req *portraitRequest) *gg.Context {
	e := *req
	e.DocTitle = "SECOND INVESTITURE"
	if e.SubLabel == "" {
		e.SubLabel = "EVOLVED"
	}
	return s10Skillup(&e)
