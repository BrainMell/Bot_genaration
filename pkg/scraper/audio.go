...
// trimClip produces the quiz theme-song 30s clip with the EXACT same ffmpeg
// arguments quiz.js (_clipAudioBuffer) used on Box 1: re-encode (not -c copy)
// so every codec lands as a clean mp3. Same binary + same args + same source
// bytes = same output bytes the old Node-side path produced.
func trimClip(src, dst string, seconds int, bitrate string) error {
	ctx, cancel := execCtx(90 * time.Second)
	defer cancel()
	cmd := exec.CommandContext(ctx, "ffmpeg", "-y", "-hide_banner", "-loglevel", "error",
		"-i", src, "-t", strconv.Itoa(seconds), "-b:a", bitrate, dst)
	cmd.Env = childEnv()
	if out, err := cmd.CombinedOutput(); err != nil {
		return fmt.Errorf("ffmpeg: %v: %s", err, tailBytes(out, 200))
	}
...
func ytDownload(id string, mp3Path string, proxy string) error {
	...
	defer cancelParent()
	clients := []string{"tv_embedded", "android_vr", "tv_simply", "mweb", "web_embedded", "tv"}
	var lastErr error
...
func ytFormatsHealthy(id string, proxy string) bool {
	for _, cl := range []string{"tv_embedded", "tv_simply", "mweb"} {
...
func ScrapeAudio(c *gin.Context) {
	...
	// P25 (quiz box-offload): optional server-side clip. quiz.js previously
	// re-downloaded the FULL mp3 and re-ran ffmpeg locally just to trim 30s;
	// passing clip_seconds hands back the clipped bytes directly. Same ffmpeg
	// binary + same args = same output bytes. Cached alongside the full file.
	audioURL := fmt.Sprintf("%s/downloads/%s.mp3", baseURL, hash)
	extra := gin.H{}
	if clipSec := c.Query("clip_seconds"); clipSec != "" {
		if n, err := strconv.Atoi(clipSec); err == nil && n > 0 && n <= 120 {
			bitrate := c.Query("clip_bitrate")
			if bitrate == "" {
				bitrate = "96k"
			}
			clipPath := fmt.Sprintf("downloads/%s_c%ds_%s.mp3", hash, n, bitrate)
			if _, err := os.Stat(clipPath); err != nil {
				if terr := trimClip(mp3Path, clipPath, n, bitrate); terr != nil {
					fmt.Printf("[Audio v3] clip trim failed (%ds %s): %v\n", n, bitrate, terr)
				}
			}
			if ci, err := os.Stat(clipPath); err == nil && ci.Size() > 20*1024 {
				if fi, err := os.Stat(mp3Path); err == nil {
					extra["fullBytes"] = fi.Size()
				}
				extra["clipped"] = true
				extra["clipSeconds"] = n
				audioURL = fmt.Sprintf("%s/downloads/%s_c%ds_%s.mp3", baseURL, hash, n, bitrate)
			}
		}
	}
