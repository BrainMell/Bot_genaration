...
func ScrapeRule34(c *gin.Context) {
	...
	// 1. Try Deep Browser Scrape First (Prioritize Browserless)
	fmt.Printf("[Rule34] Starting deep browser scrape for: %s\n", tag)
	...
	if err != nil {
		fmt.Printf("[Rule34] Browser scrape error: %v. Falling back to API...\n", err)
	}

	// 2. If Browser fails or is empty, use API Fallback
	if len(images) == 0 {
		fmt.Printf("[Rule34] Browser failed or empty, trying API fallback for: %s\n", tag)
